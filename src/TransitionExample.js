...
function Transition(sceneA, sceneB) {
  ...
  const sinWaveArray = createTransitionSinWaveArray(
    transitionParams.transitionIncrement,
    transitionParams.transitionSpeed
  );
  this.render = function(delta) {
    // Transition animation
    if (transitionParams.animateTransition) {
      transitionParams.transition = THREE.Math.smoothstep(
        sinWaveArray[transitionParams.transitionIndex],
        0.59,
        0.9999
      );
...
class TransitionExample extends Component {
  ...
  init = () => {
    ...
    var sceneA = new LandingTransition(
      renderer,
      0xffffff,
